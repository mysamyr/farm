...
function getRoomStateLabel(room: Room): HTMLSpanElement {
  return Span({
    ...
    // TODO: apply language
    text: room.state.replace(/^\w/, c => c.toUpperCase()),
  });
}
...
export function getRoomCard(room: Room): HTMLDivElement {
  ...
  const isBtnDisabled: boolean =
    isFull || isOwner || isInRoom || !canJoinState || !canJoinName;
  ...
  return Div({
    ...
    children: [
      Div({
        ...
        children: [
          Header(3, { className: 'room-name', text: room.name }),
          Div({
            className: 'room-meta',
            children: [
              Span({
...
export function getGameRules(room: Room): HTMLDivElement {
  ...
  const rulesItems = Object.values(GAME_RULES)
      ...
      if (isOwner) {
        return Div({
          ...
          children: [
            ...
            Span({ text: getRuleLabel(ruleKey) }),
          ],
        });
      }
    ...
    .filter(identity) as HTMLElement[];
  ...
  return Div({
    ...
    children: [
      ...
      Div({
        className: isOwner ? '' : 'active-rules-tags',
        children: rulesItems,
      }),
    ],
  });
