import { DEFAULT_PLAYER_STATS } from '@game/shared/constants/arena';
import type { Player, Room, StatType } from '@game/shared/types/arena';
...
export function getOpponentPlayer(room: Room): Player | undefined {
  const socketId = getSocketId();
  return room.players.find(p => p.id !== socketId);
}
