import { GAME_RULES, ROOM_STATES, TURN_START_INDEX } from '../../constants';
...
export function createRoom({
  ...
  const room: Room = {
    ...
    rules: {
      [GAME_RULES.EXTRA_DUCK]: false,
      [GAME_RULES.ONE_EXCHANGE]: false,
    },
    order: [],
...
export function updateRoomsList(io: Server): void {
  io.emit('rooms:list', listRooms().map(serializeRoom));
}
