...
import { serializeRoom } from './room.helpers';
import {
  createRoom,
  deleteRoom,
  getRoomById,
...
const updateRoomHandler =
  ...
  (req: UpdateRoomReq, ack?: AckFunc): void => {
    ...
    updateRoomsList(io);
    if (ack) ack({ ok: true, room: serializeRoom(room) });
  };
...
const closeRoomHandler =
  ...
  (req: CloseRoomReq, ack?: AckFunc): void => {
    ...
    io.to(room.id).emit('room:closed', { roomId: req.roomId });
...
export function registerRoomFeature(io: Server): void {
  io.on('connection', (socket: Socket): void => {
    ...
    socket.on('room:update', updateRoomHandler(io, socket));
    socket.on('room:join', joinRoomHandler(io, socket));
    socket.on('room:leave', leaveRoomHandler(io, socket));
    socket.on('room:close', closeRoomHandler(io, socket));
  });
}
